<?php
		...
		if($var==1)
		{
        		...
        		if($row = mysql_fetch_array($result))
					header("Location: ../registro/?nombre=$nombre&nick=$nick&mail1=$mail1&mail2=$mail2&avatar=$avatar&pais=$pais&ciudad=$ciudad&sexo=$sexo&dia=$dia&mes=$mes&ano=$ano&mensajero=$mensajero&mensaje=$mensaje&error=1");
        		else
        		{
          			$sql = "SELECT id FROM usuarios WHERE mail='".$_REQUEST["mail1"]."'";
        			$result = mysql_query($sql);
					if($row = mysql_fetch_array($result))
						header("Location: ../registro/?nombre=$nombre&nick=$nick&mail1=$mail1&mail2=$mail2&avatar=$avatar&pais=$pais&ciudad=$ciudad&sexo=$sexo&dia=$dia&mes=$mes&ano=$ano&mensajero=$mensajero&mensaje=$mensaje&error=2");
					else
					{
						if (strlen(trim($nick))<3)
							header("Location: ../registro/?nombre=$nombre&nick=$nick&mail1=$mail1&mail2=$mail2&avatar=$avatar&pais=$pais&ciudad=$ciudad&sexo=$sexo&dia=$dia&mes=$mes&ano=$ano&mensajero=$mensajero&mensaje=$mensaje&error=3");
						else
						{
							...
							$asunto="Confirmación de Extreme-Zone";
