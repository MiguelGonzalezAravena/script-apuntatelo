...
if ($id_user != '') {
  $sql = "
    ...
    WHERE id_receptor = $id_user
    AND papelera_receptor = 1
    AND (UNIX_TIMESTAMP(NOW()) - UNIX_TIMESTAMP(fecha_papelera)) > 864000";
...
?>
                <tr style="font-size: 11px; background-color: <?php echo ($row['leido_receptor'] == 0 ? '#FFF3BF;' : '#D3D3D3;'); ?>">
                  ...
                  <td width="250">
                    <a href="<?php echo $url; ?>/mensajes/mensajes_recibidos.php?mensaje=<?php echo $row['id_mensaje']; ?>" alt="Ver mensaje"><?php echo no_injection($row['asunto']); ?></a>
                  </td>
                  <td width="200"> 
                    <a href="<?php echo $url; ?>/perfil/<?php echo $row['nick']; ?>/" alt="Ver Perfil"><?php echo $row['nick']; ?></a>
                  </td>
                  <td width="150">
                    <?php echo $row['fecha']; ?>
                  </td>
...
?>
                ...
                <tr>
                  <td colspan="3" style="font-size: 10px;">
                    <br />
                    * Los mensajes se eliminar&aacute;n autom&aacute;ticamente despu&eacute;s de 10 d&iacute;as en la papelera.
                  </td>
                </tr>
              </table>
            </td>
